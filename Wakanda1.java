...
/**
 * Wakanda - Um robô atirador fujão que se esquiva dos inimigos e atira à distância.
 */
public class Wakanda extends AdvancedRobot {

    // Distâncias e variáveis de controle
    private static final double MOVE_DISTANCE = 100;  // Distância para avançar
    private static final double BACK_DISTANCE = 50;   // Distância para retroceder
    ...
    public void onScannedRobot(ScannedRobotEvent e) {
        ...
        setAhead(100);  // Move-se para frente
    }
    ...
    private void moveAndScan() {
        ...
        if (x < WALL_MARGIN || x > battlefieldWidth - WALL_MARGIN || y < WALL_MARGIN || y > battlefieldHeight - WALL_MARGIN) {
            ...
            Random rand = new Random();
            if (rand.nextBoolean()) {
                setAhead(MOVE_DISTANCE);  // Move para frente
            } else {
                setBack(BACK_DISTANCE);  // Move para trás
            }
        }
    }
